...
public class NCh031_498DiagonalTraverse对角线遍历 {
   ...
   * <p>思路
   *
   * <p>按照题目要求，模拟在数组中的行走路线，得到想要的遍历顺序。为了实现此模拟，必须清楚数组内部的行走策略。对每条对角线需要明确两件事情：
   *
   * <p>   知道该对角线的行走方向。
   *
   * <p>   确定对角线的起点元素，这取决于对角线的行走方向。
   *
   * <p>下图中标注了每条对角线的这两个信息。
   * <p>
   * <p>复杂度分析
  ...
  public int[] findDiagonalOrder_2(int[][] matrix) {
    ...
    //最初 direction 为 1，方向向上。每条对角线遍历完成后更新 direction。
    int direction = 1;
