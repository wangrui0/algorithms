 ...
 * <p>@date: 2021/5/14
 */
public class NCh051_543DiameterOfBinaryTree二叉树的直径 {

  int ans;

  /**
   * <p>   方法一：深度优先搜索(将直径转化为深度)
   * <p>   复杂度分析
   *
   * <p>     时间复杂度：O(N)，其中 NNN 为二叉树的节点数，即遍历一棵二叉树的时间复杂度，每个结点只被访问一次。
   *
   * <p>     空间复杂度：O(Height)，其中 Height 为二叉树的高度。由于递归函数在递归过程中需要为每一层递归函数分配栈空间，所以这里需要额外的空间且该空间取决于递归的深度，
   * <p>     而递归的深度显然为二叉树的高度，并且每次递归调用的函数里又只用了常数个变量，所以所需空间复杂度为 O(Height) 。
   *
   * @param root
   * @return
   */
